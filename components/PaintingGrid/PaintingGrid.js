...
const PaintingGrid = ({ paintings = [], filterTag = "" }) => {
  return (
    <div className="grid min-h-screen grid-cols-3 lg:grid-cols-8 auto-rows-min">
      {paintings
        ...
        .map((p) => {
          ...
          return (
            <div
              className={clsx(
                "relative w-full h-48 lg:h-60 col-span-1 lg:col-span-1 focus:outline-none group focus-within:ring focus-within:ring-highlight focus-within:z-10",
                !isShow && "opacity-10"
              )}
              key={_id}
            >
