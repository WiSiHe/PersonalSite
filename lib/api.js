...
export async function getAllVideos() {
  const results = await getClient().fetch(
    `*[_type == "video"]{title,thumbnail, content,linkedPainting, videoUrl, _id, "tags": tags[]->{name}}`
  )
  return results
}
...
export async function getProjectDetails(slug) {
  const results = await getClient().fetch(
    `*[_type == "project" && slug.current == $slug]{title, description, extraImages, projectStart, projectEnd, status, content, name, slug, image, slug, _id, connectedVideo->{videoUrl}, connectedPaintings[]->{title, slug, image}, tags[]->{name}}`,
    {
      slug,
    }
  )
