...
export async function getProjectDetails(slug) {
  const results = await getClient().fetch(
    `*[_type == "project" && slug.current == $slug]{title, description, projectStart, projectEnd, status, content, name, slug, image, slug, _id,connectedPaintings[]->{title, slug, image}, tags[]->{name}}`,
    {
      slug,
    }
  )
