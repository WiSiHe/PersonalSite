...
export async function getPaintingDetails(slug) {
  const results = await client.fetch(
    `*[_type == "painting" && slug.current == $slug]{
      title, description, format, image, redbubbleUrl, _id, images, "tagCount": count(tagsV2), "imagesCount": count(images),tagsV2[]->{name}
    }`,
    { slug }
  )
  return results
...
export async function getAllTags(preview) {
  const results = await getClient(preview).fetch(`*["tags"]`)
  return results
}
