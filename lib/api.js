...
export async function getAllPostsForHome() {
  const results = await getClient().fetch(`*[_type == "post"] | order(publishedAt desc){
      ${postFields}
    }`)
...
export async function getPainting(slug) {
  const results = await getClient().fetch(`*[_type == "painting" && slug.current == $slug]`, {
    slug
  })
  return results
}
...
export async function getAllTags() {
  const results = await getClient().fetch(`*[_type == "tag"]{name}`)
  return results
}
...
export async function getAllTagsAndPaintingsLight() {
  const paintingQuery = /* groq */ `*[_type == "painting"]{
    title, format, image, slug, redbubbleUrl, _id ,tagsV2[]->{name}, video
  }`
  const tagsQuery = /* groq */ `*[_type == "tag"]| order(name asc){name}`
...
export async function getAllVideos() {
  const results = await getClient().fetch(`*[_type == "video"]`)
  return results
}
